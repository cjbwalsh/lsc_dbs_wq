...
runoff and the restorative effects of SCMs into two comparable measures: *EI~S1~*, the proportion of catchment connected to stormwater drainage pipes assuming SCMs have no effect; and *EI~S~*, EI weighted by the stormwater impact index, which measures the cumulative modeled performance of upstream SCMs. Such an approach permits comparison of degradation and restoration trajectories in response to conventional drainage and stormwater control, respectively. It also presents an approach that could be applied across regions and cities for better comparison of studies.
...
temperate south-eastern Australia (Figure 1a). The study area experiences a warm temperate climate with warm summers and
...
to 2012 [@WalshEtAl_2022]. The SCMs were implemented through several engagement programs in the L4 catchment [@bos2015;@burns2015; @walsh2015], and in a parallel program in the D8 catchment led by Melbourne Water [@prosser2015]. In all
...
downpipe diverters were also installed on many houses (Figure 1c), but these were found to have little capacity to retain
...
catchments (2001 vs 2019 EI~S1~ in Table 1). SCM implementation in the 6 experimental catchments (Figure 1) reduced EI by
...
```{r Fig_2, fig.width = 6.5, fig.height = 3}
...
For each response variable, the model with maximum ELPD~loo~ was selected for assessment (Text S1, Table S1). A clearly best model (the primary model, eq. 3, plus *septic*, *season*, *rain365* and an interaction between *septic* and rain1) was evident for NO~x~ (Table S1). The same model structure was the best fit for TN, but excluding the *season *effect reduced fit only slightly. For temperature, models including *season*, *diel*, and interaction terms between the *season* effect and *restr* and *degrd* were the best fit, and inclusion of *rain365* or interaction terms between the *diel* effect and *restr* and *degrd* only slightly improved fit. For other variables, addition of *season*, *rain365* and *channel* to the primary model (Eq. 3) made only small differences to model fit. Additional variables made no consequential difference to the effects of the predictors of primary interest (*restr, degrd, rain1* and interactions). Models selected for assessment were:
...
# tiff("images/Fig 3.tiff", width = 6*600, height = 8*600, res = 600, compression = "lzw")
vars <- c("tp","frp","tn","nox","nh3","tem","ec","tss")
...
# tiff("images/Fig 4.tiff", width = 5*600, height = 8*600, res = 600, compression = "lzw")
lo <- layout(matrix(c(1:8,9,9),5,2,byrow=TRUE), 
             widths = c(10,10), heights = c(rep(10,3),11,1))
...
# tiff("images/Fig 6.tiff", width = 5*600, height = 8*600, res = 600, compression = "lzw")
...
Stormwater drainage is as much a dry weather problem as a rain-related problem. The conventional approach to stormwater drainage manages the risk of urban flooding, but in doing so, makes the risk of downstream environmental damage a certainty. SCMs installed in this experiment had the greatest influence on stream water quality during dry weather. There were two likely causes for this effect. First, baseflows, diminished by covering of catchment soils by impervious surfaces, were likely augmented by SCMs, as evidenced by reduced temperatures and increased EC. While some of this effect may be filtered flows through the pipe network, and exfiltrated flows through gravel-filled trenches and other elements of the urban karst [@bonneau2018], increased EC suggests that exfiltrated water from SCMs (with low EC, see below) increased the flow of saline groundwater into the streams. These increased groundwater flows likely reduced nutrient concentrations and temperatures of stream baseflows.Second, where we were able to install SCMs that intercepted runoff from entire sub-catchments of pipes that convey runoff from roofs and roads, SCMs prevented dry-weather spills from flowing to the stream. This was true even for the several end-of-pipe SCMs that were suboptimal in that they only had the capacity to retain and treat runoff effectively up to 1-2 mm of rain, but by providing dry-weather interception, they effectively intercepted any dry-weather spills that would otherwise have flowed to the stream.

The saline groundwater in the study catchments is likely a legacy of historic septic tank seepage, and possibly of pre-urban agricultural activities. (Road salting is not practised in the study area.) Most properties in the experimental catchments were sewered between 1985 and 1995 [@sew_2020; @yvw_2020], but some septic tanks remain in all catchments, as the backlog of sewer connections remains an ongoing endeavor [@vicauditgen_2006]. As stormwater flowing through the SCMs, like stormwater runoff in most of Australia [@gernjak_etal_2017], was much less saline than the receiving groundwater in the L4 catchment, it is therefore possible that the SCM-induced increase in EC is temporary, and the legacy salts may flush out in the future. The low EC of reference streams with high septic tank density (Table 1, Figure 4g) suggests strong connections between groundwater and stream flow can prevent accumulation of salts in catchment soils.  
...
Despite the complications of septic tank and wastewater discharges in the study catchments, our results indicate that urban stormwater drainage increases P concentrations, EC and stream temperature in dry-weather and after small-to-moderate rain events (higher values in urban streams compared to reference streams with similar or higher septic tank densities).  The experimental SCMs reduced P concentrations and stream temperature, most strongly during dry weather, by intercepting any polluted dry-weather stormwater drainage flows and increasing groundwater flows with stormwater that had been filtered through the SCMs and catchment soils.  These mitigating effects of SCMs reduced with increased rainfall because of the increased likelihood that their storage capacities were exceeded.
...
capacity to minimise overflow or bypass. 

The stormwater management principle that SCMs should be applied to all catchment impervious surfaces for protection of streams [@walsh2016] was predicated on the non-linear response of stream ecosystems to EI, meaning that small areas of connected impervious surfaces in a catchment potentially have a large influence on stream degradation.  In our study, substantial reductions in P and N concentrations, and in temperature were only achieved in those two sites where SCMs treated runoff from almost all impervious surfaces (L4 and Ls), thus permitting substantial reductions in *EI~S~*. However, even in those sites, reductions after more than ~8 mm were limited because the performance of many of the SCMs, as measured by S, the stormwater impact index used to weight  *EI~S~*, was suboptimal.  Thus, while interception of runoff from all catchment impervious surfaces is likely a requirement for stream protection, the intercepting SCMs need to have high performance standards. Suboptimal SCM design and performance is likely to also explain the only partial protection of the streams fully treated by SCMs in the study described by @hopkins_etal_2022.  

The performance metrics underpinning *S* and *EI~S~*, require sufficient retention capacity to minimize the frequency of uncontrolled runoff (bypass or overflow), and filtered flow rates and volumes that match stream baseflow in the catchment’s pre-urban, vegetated state [@WalshEtAl_2022]. Because impervious surfaces generate substantially more runoff than pervious land [@walsh2012], achieving both of these aims requires loss, through harvesting or evapotranspiration (principle 2 of @walsh2016).  Relying solely on infiltration systems is likely to result in unnaturally high groundwater flows, with concomitant pollution risks.  The volumes of filtered runoff achieved in Ln and D8 were within the target range, but in all sites, the volumes lost through harvesting or evapotranspiration were well below the target range [@WalshEtAl_2022].

The experimental SCMs thus fell short in two ways: insufficient coverage of impervious surfaces in all but two catchments, and insufficient
SCM performance. Addressing these two shortcoming requires an ambitious and determined approach to SCM implementation.
...
Achieving both reduction in uncontrolled runoff frequency and restored filtered flow regime depends on the amount of demand for harvested stormwater [@walsh2016], as well as maximizing opportunities for evapotranspiration. Maximizing the amount of demand requires end-uses which are regular (e.g. indoor uses) rather than seasonal (e.g. irrigation). Without these regular demands, rainwater and stormwater
...
performance in its formulation [@WalshEtAl_2022], it can be used to assess the effects of a wide range of potential SCM types, designs and implementation strategies, to predict in-stream water quality responses and prioritize management actions. By its nature,*EI~S~* takes into account the local climate and hydrology, thus making is suitable for application in a broad range of regions.
...
predicted by estimation of the resulting *EI~S~*. However, the less certain achievement of the TN objective (Figure 4c) and
...
likely a legacy of historic septic tanks or perhaps pre-urban agriculture. Infiltration of urban stormwater runoff (which is less saline, as evidenced by reduced EC with higher rainfall) into catchment soils is likely to result in long-term dilution of groundwater flows, over longer time frames than was possible in this study. SCMs are therefore most likely an appropriate tool for addressing this legacy problem for a
...
Protection and ecologically successful restoration of river and stream ecosystems are likely to require approaches to stormwater control beyond those that currently dominate [@walsh2016]. If river and stream protection is an objective, more ambitious targets are required than, for instance, those of most Mid-Atlantic municipalities, which have a goal of achieving 10--20% of the landscape drain runoff through SCMs by 2030 [@pennino2016]. Stream protection will require changing standard drainage practice so that conventional drainage is no longer the default. It will require near all impervious surfaces draining to SCMs with high performance standards, noting that, because of the much greater frequency and volume of runoff from impervious surfaces than from pervious surfaces (Walsh et al. 2012), SCMs should be designed for interception of runoff from impervious surfaces, not from entire landscapes: cf @pennino2016). It will require SCMs in treatment trains at a range of scales from land-parcel to the largest sub-catchment that enters a stream, and with the terminal treatment supported as much as possible by upstream treatments.
...
| effect)**]{.underline}                                    |                                                           |
+-----------------------------------------------------------+-----------------------------------------------------------+
| 7.  To stormwater drains                                  | Yes: reduced N                                            |
+-----------------------------------------------------------+-----------------------------------------------------------+
| 8.  To groundwater                                        | Possibly: reduced N                                       |
+-----------------------------------------------------------+-----------------------------------------------------------+
| [**CLIMATE CHANGE (*time* effect)**]{.underline}          |                                                           |
+-----------------------------------------------------------+-----------------------------------------------------------+
| 9.  Potential reduced dry weather flows                   | Yes: likely increased dry-weather flows inferred from   |
|                                                           | reduced dry-weather temperature, increased EC             |
+-----------------------------------------------------------+-----------------------------------------------------------+
| 10.  Warming                                              | Yes: reduced temperature                                  |
+-----------------------------------------------------------+-----------------------------------------------------------+
...
Programs for planning, allocating, building and maintaining SCMs in the L4 catchment were
