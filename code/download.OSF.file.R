...
## copied from https://osf.io/hkpjb, 
##  CJW added subdir argument, to permit download to subdirectory (as a text string)
...
download.OSF.file <- function(GUID,Access_Token=NULL,file_name, subdir="")
{
  require(httr)
  require(rjson)
  require(here)
  ...
  if (length(json_data$data) > 0){
          req1 <- GET(json_data$data$links$download,
                      write_disk(here(subdir,file_name), overwrite = TRUE))
